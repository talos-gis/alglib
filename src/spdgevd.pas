...
unit spdgevd;
interface
uses Math, Sysutils, Ap, cholesky, sblas, blas, trinverse, rotations, tdevd, reflections, tridiagonal, sevd;
...
function GeneralizedSymmetricDefiniteEVD(A : TReal2DArray;
     N : AlglibInteger;
     IsUpperA : Boolean;
     const B : TReal2DArray;
     IsUpperB : Boolean;
     ...
     var D : TReal1DArray;
     var Z : TReal2DArray):Boolean;
function GeneralizedSymmetricDefiniteEVDReduce(var A : TReal2DArray;
     N : AlglibInteger;
     IsUpperA : Boolean;
     const B : TReal2DArray;
     IsUpperB : Boolean;
...
var
    ...
    i_ : AlglibInteger;
begin
    Assert(N>0, 'GeneralizedSymmetricDefiniteEVDReduce: N<=0!');
    Assert((ProblemType=1) or (ProblemType=2) or (ProblemType=3), 'GeneralizedSymmetricDefiniteEVDReduce: incorrect ProblemType!');
    Result := True;
    ...
    begin
        ...
        begin
            ...
            while I<=N do
            begin
                APVMove(@T[I][0], 1, I, @B[I][0], 1, I);
                Inc(I);
            end;
        end;
        if  not CholeskyDecomposition(T, N, False) then
        begin
            Result := False;
            Exit;
        end;
        ...
        // Invert L in T
        //
        if  not InvTriangular(T, N, False, False) then
        begin
            Result := False;
            Exit;
        end;
        ...
        begin
            ...
            while I<=N do
            begin
                V := APVDotProduct(@T[I][0], 1, I, @W2[0], 1, I);
                R[I,J] := V;
                Inc(I);
            end;
            Inc(J);
    ...
    begin
        ...
        if  not CholeskyDecomposition(T, N, True) then
        begin
            Result := False;
            Exit;
        end;
        ...
        begin
            ...
            // Form w2 = A * u'(j) (here u'(j) is j-th column of U')
            //
            APVMove(@W1[0], 1, N-J+1, @T[J][0], J, N);
            SymmetricMatrixVectorMultiply(A, IsUpperA, J, N, W1, 1.0, W3);
            APVMove(@W2[0], J, N, @W3[0], 1, N-J+1);
            APVMove(@W1[0], J, N, @T[J][0], J, N);
            if IsUpperA then
            begin
            ...
            while I<=N do
            begin
                V := APVDotProduct(@T[I][0], I, N, @W2[0], I, N);
                R[I,J] := V;
                Inc(I);
            end;
            Inc(J);
        ...
        begin
            ...
            // Invert U in T
            //
            if  not InvTriangular(T, N, True, False) then
            begin
                Result := False;
                Exit;
            end;
